				       ...
				       __s8 *ext_err __maybe_unused)
{
	...
	if (!decrypted) {
		...
		send_trace_notify_flags(ctx, TRACE_TO_STACK, *identity,
					...
					TRACE_REASON_UNKNOWN, 0, CLS_FLAG_ENCRYPTED);
 ...
 *
 * 1. will be traced with CLS_FLAG_ENCRYPTED
 * 2. will be traced without CLS_FLAG_ENCRYPTED
 * 3. will be traced without CLS_FLAG_ENCRYPTED
 *
 * Note that 1. contains the ESP packets someone else generated.
 * In that case, we trace it as "encrypted", but it doesn't mean
 * "encrypted by Cilium".
 *
 * When IPSec is disabled, we won't use CLS_FLAG_ENCRYPTED even

 ...
 *
 * 1. will be traced with CLS_FLAG_ENCRYPTED, because
 * do_decrypt marks them with MARK_MAGIC_DECRYPT.
 *
 * 2. will be traced without CLS_FLAG_ENCRYPTED, because
 * do_decrypt does't touch to mark.
 *
 * 3. will be traced without CLS_FLAG_ENCRYPTED, because
 ...
 * "encrypted by Cilium".
 *
 * We won't use CLS_FLAG_ENCRYPTED even if the packets are ESP,
 * because it doesn't matter for the non-IPSec mode.
 */
#ifdef ENABLE_IPSEC
	if ((ctx->mark & MARK_MAGIC_HOST_MASK) == MARK_MAGIC_DECRYPT)
		trace.flags = CLS_FLAG_ENCRYPTED;
